...
//		$shopPass = '5wabpahf';
		...
		require_once '/home/fujii-office/www/op/src/config.php';

		if(!empty( $insert_id )) {
			require_once '/home/fujii-office/www/op/src/com/gmo_pg/client/input/EntryTranInput.php';
			require_once '/home/fujii-office/www/op/src/com/gmo_pg/client/input/ExecTranInput.php';
			require_once '/home/fujii-office/www/op/src/com/gmo_pg/client/input/EntryExecTranInput.php';
			require_once '/home/fujii-office/www/op/src/com/gmo_pg/client/tran/EntryExecTran.php';
			...
			if( $exe->isExceptionOccured() ){//取引の処理そのものがうまくいかない（通信エラー等）場合、例外が発生します。
				...
				require_once('/home/fujii-office/www/op/src/sample/display/Exception.php');
				exit;
				...
				if( $output->isErrorOccurred() ){//出力パラメータにエラーコードが含まれていないか、チェックしています。
					...
					require_once( '/home/fujii-office/www/op/src/sample/display/EntryExecError.php');
					exit;
					...
					require_once( '/home/fujii-office/www/op/src/com/gmo_pg/client/input/AcsParam.php');
					require_once( '/home/fujii-office/www/op/src/com/gmo_pg/client/common/RedirectUtil.php');
...
//		$shopPass = '5wabpahf';
		...
		$jobCd = 'VOID'; // 取り消し
		if(!empty($query)) {
			require_once '/home/fujii-office/www/op/src/com/gmo_pg/client/input/AlterTranInput.php';
			require_once '/home/fujii-office/www/op/src/com/gmo_pg/client/tran/AlterTran.php';
			...
			if( $exe->isExceptionOccured() ){//取引の処理そのものがうまくいかない（通信エラー等）場合、例外が発生します。
				...
				//サンプルでは、例外メッセージを表示して終了します。
				require_once( '/home/fujii-office/www/op/sample/display/Exception.php');
				exit();
				...
				if( $output->isErrorOccurred() ){//出力パラメータにエラーコードが含まれていないか、チェックしています。
					...
					//サンプルでは、エラーが発生していた場合、エラー画面を表示して終了します。
					require_once( '/home/fujii-office/www/op/sample/display/Error.php');
					exit();
...
//		$shopPass = '5wabpahf';
		...
		$jobCd = 'CAPTURE'; // 即時売上
		if(!empty($query)) {
			require_once( '/home/fujii-office/www/op/src/com/gmo_pg/client/input/ChangeTranInput.php');
			require_once( '/home/fujii-office/www/op/src/com/gmo_pg/client/tran/ChangeTran.php');
			//入力パラメータクラスをインスタンス化します
			...
			if ( $exe->isExceptionOccured() ){//取引の処理そのものがうまくいかない（通信エラー等）場合、例外が発生します。
				...
				if( $output->isErrorOccurred() ){//出力パラメータにエラーコードが含まれていないか、チェックしています。
					//サンプルでは、エラーが発生していた場合、エラー画面を表示して終了します。
					require_once( '/home/fujii-office/www/op/src/sample/display/Error.php');
					exit();
				}
			}
			//例外発生せず、エラーの戻りもなく、3Dセキュアフラグもオフであるので、実行結果を表示します。
